...
const AddOrderModal = ({ isVisible, onCancel, onSubmit }) => {
    ...
    const handleFinish = async (values) => {
        const validItems = products.filter(item => item.productId !== null); // Bỏ các item thiếu productId
        ...
        try {
            ...
            await axios.post("http://localhost:4000/orders", orderData); // Send the order data to the backend

            onSubmit(); // Call any additional submit handling
            form.resetFields(); // Reset the form fields
            setProducts([{ id: Date.now(), productId: null, quantity: 1, price: 0 }]); // Reset products array
